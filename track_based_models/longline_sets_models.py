...
class LonglineSetsModel11(SingleTrackModel):
    ...
    def __init__(self, width=None):
        ...
        def pool_block(y, width):
            # Would like to use cropped, but may need more data
            y1 = MaxPooling1D(pool_size=width, strides=1)(y)
            y2 = Cropping1D(width // 2)(y)
            y = keras.layers.concatenate([y1, y2])
            return y

        input_layer = Input(shape=(width, 6)) #19
        y = Conv1D(base_filter_count, 1)(input_layer)
...
class LonglineSetsModel12(SingleTrackModel):
    # https://arxiv.org/pdf/1905.05928.pdf
    ...
    def __init__(self, width=None):
        ...
        y = pool_block(shake_shake_block(y, 1.0 * base_filter_count, 1), y, 5)

        y = pool_block(shake_shake_block(y, 1.8 * base_filter_count, 2), y, 9)
        y = pool_block(shake_shake_block(y, 1.8 * base_filter_count, 2), y, 9)

        y = pool_block(shake_shake_block(y, 2.6 * base_filter_count, 4), y, 17)
        y = pool_block(shake_shake_block(y, 3.4 * base_filter_count, 4), y, 17)

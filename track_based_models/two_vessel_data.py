...
all_paths = (glob('./labeled_transshipment_tracks/*.json') + 
             glob('./good_new_labeled_tracks/*.json'))
...
def build_features(obj, delta=None, interp_t=None, skip_label=False, keep_frac=1.0):
    ...
    v[np.isnan(v)] = 3.5
    obj['sogs'] = v
    xi, speeds = lin_interp(obj, 'sogs', delta=delta, t=interp_t, mask=mask)
    y0 = speeds
    #
    _, cos_yi = lin_interp(obj, 'courses', delta=delta, t=interp_t, mask=mask, func=cos_deg)
    _, sin_yi = lin_interp(obj, 'courses', delta=delta, t=interp_t, mask=mask, func=sin_deg)
    angle_i = np.arctan2(sin_yi, cos_yi)
    y1 = angle_i
    #
    _, y2 = lin_interp(obj, 'lats', delta=delta, t=interp_t, mask=mask)
    # Longitude can cross the dateline, so interpolate useing cos / sin
    _, cos_yi = lin_interp(obj, 'lons', delta=delta, t=interp_t, mask=mask, func=cos_deg)
    _, sin_yi = lin_interp(obj, 'lons', delta=delta, t=interp_t, mask=mask, func=sin_deg)
...
def load_paired_data(path, delta, skip_label=False, keep_frac=1):
    obj_tv = util.load_data(path, vessel_label= 'position_data_reefer')  
    obj_fv = util.load_data(path, vessel_label= 'position_data_fishing_vessel')

    t, x, y_tv, label, is_defined = build_features(obj_tv, delta=delta, 
                                            skip_label=skip_label, keep_frac=keep_frac)
    t_fv, x_fv, y_fv, _, _ = build_features(obj_fv, interp_t = t, 
                                             skip_label=True, keep_frac=keep_frac)
    
    assert np.all(t_fv == t)
    
    t = np.asarray(t)

    return (t, x, y_tv, x_fv, y_fv, label, is_defined)
    

def cook_paired_data(t, x, y_tv, x_fv, y_fv, label, is_defined, start_ndx=0, end_ndx=None):
    ...
    features_fv, _, _, _ = cook_features(y_fv, angle=angle,
                                                mean_2=mean_2, mean_3=mean_3)
    # Relative speed
...
def generate_data(sets, window, delta, min_samples, label_window=None, seed=888, skip_label=False,
    ...
    for p in paths:
        for kf in keep_fracs:
            ...
            paired_data = load_paired_data(p, delta, skip_label, kf)
            (t_tv, x_tv, y_tv, x_fv, y_fv, label_tv, defined_tv) = paired_data
                ...
                for trys_left in reversed(range(KEEP_TRYS)):
                    ...
                    if force_local and trys_left > 0:
                        if use_local == bool(label_tv[ndx:ndx+window_pts].sum()):
                            continue
                    t_chunk, f_chunk = cook_paired_data(*paired_data, start_ndx=ndx, 

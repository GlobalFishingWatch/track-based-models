...
def _create_features_and_times(mdl, data, angle=77):
    ...
    for i in range(min_ndx, max_ndx):
        ...
        features.append(_cook_features(raw_features, angle=angle, noise=0)[0])
...
def predict_set_times(mdl, data):
    ...
    for angle in [77, 167, 180, 270]:
        features, times = _create_features_and_times(mdl, data, angle=angle)
